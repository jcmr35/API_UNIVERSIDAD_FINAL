      ...
      db.query(queryVerificarReserva, [usuarioId, hoyStr, tipoComida], (err, reservasExistentes) => {
        ...
        db.query(queryAsegurarCupos, [hoyStr, tipoComida, cuposTotalesDefault, cuposTotalesDefault], (err, resultInsert) => {
           ...
           db.query(querySeleccionarCupos, [hoyStr, tipoComida], (err, resultsCupos) => {
              if (err) { console.error("Error seleccionando/bloqueando cupos (TX):", err); return db.rollback(() => res.status(500).json({ status: 'error', message: 'Error DB obteniendo cupos' })); }
              if (resultsCupos.length === 0) { console.error("Error crítico: Fila de cupos no encontrada."); return db.rollback(() => res.status(500).json({ status: 'error', message: 'Error interno (cupos)' })); }
              const cuposDisponibles = resultsCupos[0].cupos_disponibles;
              if (cuposDisponibles <= 0) { return db.rollback(() => res.json({ status: 'error', message: `No hay cupos disponibles para ${tipoComida}` })); }
              ...
              db.query(queryReservar, [usuarioId, hoyStr, tipoComida], (err, resultReserva) => {
                 ...
                 db.query(queryDecrementar, [hoyStr, tipoComida], (err, resultUpdate) => {
                    ...
                    if (resultUpdate.affectedRows > 0) {
                       ...
                       console.warn("No se pudo decrementar cupo (posible concurrencia o 0 cupos).");
...
app.get('/noticias', (req, res) => {
  // ... (Tu ruta de noticias sin cambios) ...
  const query = 'SELECT id, titulo, imagen_url, enlace_url FROM noticias ORDER BY id DESC LIMIT 5'; db.query(query, (err, results) => { if (err) { console.error('Error al consultar noticias:', err); return res.status(500).json({ status: 'error', message: 'Error del servidor al buscar noticias' }); } console.log(`Enviando ${results.length} noticias.`); const noticiasFormateadas = results.map(noticia => ({ id: noticia.id, title: noticia.titulo, imageUrl: noticia.imagen_url, linkUrl: noticia.enlace_url })); res.json({ status: 'success', noticias: noticiasFormateadas }); });
});
// --- FIN RUTA NOTICIAS ---

// --- RUTA: OBTENER CURSOS DE UN USUARIO ---
app.get('/cursos/:usuarioId', (req, res) => {
  // ... (Tu ruta de cursos sin cambios) ...
  const usuarioId = req.params.usuarioId; console.log(`Petición recibida para /cursos/${usuarioId}`); const query = ` SELECT DISTINCT c.id as curso_id, c.nombre_curso, COALESCE(MAX(r.aula), 'virtual') as aula FROM reglas_horario r JOIN cursos c ON r.curso_id = c.id WHERE r.usuario_id = ? GROUP BY c.id, c.nombre_curso ORDER BY c.nombre_curso; `; db.query(query, [usuarioId], (err, results) => { if (err) { console.error('Error al consultar cursos del usuario:', err); return res.status(500).json({ status: 'error', message: 'Error del servidor al buscar cursos' }); } console.log(`Enviando ${results.length} cursos para el usuario.`); const cursosFormateados = results.map(curso => ({ id: curso.curso_id, name: curso.nombre_curso, location: curso.aula })); res.json({ status: 'success', cursos: cursosFormateados }); });
});
// --- FIN RUTA CURSOS ---
